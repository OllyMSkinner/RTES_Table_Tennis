...
namespace icm20948
{
    ...
    bool ICM20948_I2C::read_sample(IMUSample& sample)
    {
        ...
        // Magnetometer is optional for your upright logic.
        // Keep trying, but don't fail the whole sample if magn read fails.
        if (!read_magn()) {
            magn[0] = magn[1] = magn[2] = 0.0f;
        }
    ...
    bool ICM20948_I2C::_set_bank(uint8_t bank)
    {
        ...
        if (_i2c.writeReg(ICM20948_REG_BANK_SEL_ADDR, static_cast<unsigned>(bank_byte_value)) == mraa::SUCCESS) {
            _current_bank = bank;
            return true;
        }
    ...
    {
        bool success = _write_mag_byte(AK09916_CNTL2_ADDR, static_cast<uint8_t>(icm20948::MAGN_SHUTDOWN));

        if (success) {
            success &= _write_mag_byte(AK09916_CNTL2_ADDR, static_cast<uint8_t>(settings.magn.mode));
        }
    ...
    bool ICM20948_I2C::_write_bit(const uint8_t bank, const uint8_t reg, const uint8_t bit_pos, const bool bit)
    {
        ...
        if (success) {
            uint8_t new_byte =
                static_cast<uint8_t>((preexisting_byte & ~static_cast<uint8_t>(1U << bit_pos)) |
