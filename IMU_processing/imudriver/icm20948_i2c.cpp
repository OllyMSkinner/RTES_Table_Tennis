...
namespace icm20948
{
    ...
    bool ICM20948_I2C::init()
    {
        ...
        success &= _read_byte(ICM20948_WHO_AM_I_BANK,
                              ICM20948_WHO_AM_I_ADDR,
                              device_id);
    ...
    bool ICM20948_I2C::reset()
    {
        ...
        success &= _write_bit(ICM20948_PWR_MGMT_1_BANK,
                              ICM20948_PWR_MGMT_1_ADDR,
                              7,
                              true);
    ...
    bool ICM20948_I2C::read_accel_gyro()
    {
        ...
        if (!_read_block_bytes(ICM20948_ACCEL_OUT_BANK,
                               ICM20948_ACCEL_XOUT_H_ADDR,
                               buf,
                               12)) {
            return false;
        }

        uint8_t inverted_buf[12] = {};
        for (int i = 0; i < 12; ++i) {
            inverted_buf[i] = buf[11 - i];
        }

        int16_t* gyro_raw  = reinterpret_cast<int16_t*>(inverted_buf);
        int16_t* accel_raw = reinterpret_cast<int16_t*>(inverted_buf + 6);
    ...
    bool ICM20948_I2C::read_magn()
    {
        ...
        if (!_read_block_bytes(ICM20948_EXT_SLV_SENS_DATA_00_BANK,
                               ICM20948_EXT_SLV_SENS_DATA_00_ADDR,
                               buf,
                               6)) {
            return false;
        }
    ...
    bool ICM20948_I2C::_set_bank(uint8_t bank)
    {
        ...
        uint8_t bank_byte_value = 0;
        switch (bank) {
            case 0: bank_byte_value = ICM20948_REG_BANK_SEL_BANK0_VALUE; break;
            case 1: bank_byte_value = ICM20948_REG_BANK_SEL_BANK1_VALUE; break;
            case 2: bank_byte_value = ICM20948_REG_BANK_SEL_BANK2_VALUE; break;
            case 3: bank_byte_value = ICM20948_REG_BANK_SEL_BANK3_VALUE; break;
            default:
                throw std::runtime_error("Invalid bank number in _set_bank(): " + std::to_string(bank));
        }

        if (_i2c.writeReg(ICM20948_REG_BANK_SEL_ADDR,
                          static_cast<unsigned>(bank_byte_value)) == mraa::SUCCESS) {
            _current_bank = bank;
            return true;
        }
    ...
    bool ICM20948_I2C::_set_accel_sample_rate_div()
    {
        ...
        bool success = true;
        success &= _write_byte(ICM20948_ACCEL_SMPLRT_DIV_1_BANK,
                               ICM20948_ACCEL_SMPLRT_DIV_1_ADDR,
                               msb);
        success &= _write_byte(ICM20948_ACCEL_SMPLRT_DIV_2_BANK,
                               ICM20948_ACCEL_SMPLRT_DIV_2_ADDR,
                               lsb);
    ...
    bool ICM20948_I2C::_set_accel_range_dlpf()
    {
        ...
        bool success = _write_byte(ICM20948_ACCEL_CONFIG_1_BANK,
                                   ICM20948_ACCEL_CONFIG_1_ADDR,
                                   byte);
        if (success) {
    ...
    bool ICM20948_I2C::_set_gyro_range_dlpf()
    {
        ...
        bool success = _write_byte(ICM20948_GYRO_CONFIG_1_BANK,
                                   ICM20948_GYRO_CONFIG_1_ADDR,
                                   byte);
        if (success) {
    ...
    bool ICM20948_I2C::_enable_data_ready_interrupt()
    {
        ...
        success &= _write_byte(ICM20948_INT_PIN_CFG_BANK,
                               ICM20948_INT_PIN_CFG_ADDR,
                               0x00);

        success &= _write_byte(ICM20948_INT_ENABLE_1_BANK,
                               ICM20948_INT_ENABLE_1_ADDR,
                               0x01);
    ...
    {
        bool success = _write_bit(ICM20948_INT_PIN_CFG_BANK,
                                  ICM20948_INT_PIN_CFG_ADDR,
                                  1,
                                  false);

        if (success) {
            success &= _write_byte(ICM20948_I2C_MST_CTRL_BANK,
                                   ICM20948_I2C_MST_CTRL_ADDR,
                                   0x17);
        }

        if (success) {
            success &= _write_bit(ICM20948_USER_CTRL_BANK,
                                  ICM20948_USER_CTRL_ADDR,
                                  5,
                                  true);
        }
    ...
    {
        bool success = _write_mag_byte(AK09916_CNTL2_ADDR,
                                       static_cast<uint8_t>(icm20948::MAGN_SHUTDOWN));

        if (success) {
            success &= _write_mag_byte(AK09916_CNTL2_ADDR,
    ...
    {
        bool success = _write_byte(ICM20948_I2C_SLV0_ADDR_BANK,
                                   ICM20948_I2C_SLV0_ADDR_ADDR,
                                   0x8C);
        std::this_thread::sleep_for(std::chrono::milliseconds(5));

        if (success) {
            success &= _write_byte(ICM20948_I2C_SLV0_REG_BANK,
                                   ICM20948_I2C_SLV0_REG_ADDR,
                                   0x11);
            std::this_thread::sleep_for(std::chrono::milliseconds(5));
        }

        if (success) {
            success &= _write_byte(ICM20948_I2C_SLV0_CTRL_BANK,
                                   ICM20948_I2C_SLV0_CTRL_ADDR,
                                   0x89);
            std::this_thread::sleep_for(std::chrono::milliseconds(5));
        }
    ...
    bool ICM20948_I2C::_write_byte(const uint8_t bank,
                                   const uint8_t reg,
                                   const uint8_t byte)
    {
    ...
    bool ICM20948_I2C::_read_byte(const uint8_t bank,
                                  const uint8_t reg,
                                  uint8_t& byte)
    {
        uint8_t ret = 0;
    ...
    bool ICM20948_I2C::_write_bit(const uint8_t bank,
                                  const uint8_t reg,
                                  const uint8_t bit_pos,
                                  const bool bit)
    {
        uint8_t preexisting_byte = 0;
        bool success = _read_byte(bank, reg, preexisting_byte);

        if (success) {
            uint8_t new_byte =
                static_cast<uint8_t>(
                    (preexisting_byte & ~static_cast<uint8_t>(1U << bit_pos)) |
    ...
    bool ICM20948_I2C::_read_bit(const uint8_t bank,
                                 const uint8_t reg,
                                 const uint8_t bit_pos,
                                 bool& bit)
    {
        ...
        if (success) {
            bit = ((preexisting_byte >> bit_pos) & static_cast<uint8_t>(1U)) != 0U;
        }

        return success;
    }

    bool ICM20948_I2C::_read_block_bytes(const uint8_t bank,
    ...
    bool ICM20948_I2C::_write_mag_byte(const uint8_t mag_reg,
                                       const uint8_t byte)
    {
        bool success = true;

        if (success) {
            success &= _write_byte(ICM20948_I2C_SLV4_ADDR_BANK,
                                   ICM20948_I2C_SLV4_ADDR_ADDR,
                                   ICM20948_MAGN_I2C_ADDR);
        }

        if (success) {
            success &= _write_byte(ICM20948_I2C_SLV4_REG_BANK,
                                   ICM20948_I2C_SLV4_REG_ADDR,
                                   mag_reg);
        }

        if (success) {
            success &= _write_byte(ICM20948_I2C_SLV4_DO_BANK,
                                   ICM20948_I2C_SLV4_DO_ADDR,
                                   byte);
        }

        if (success) {
            success &= _write_byte(ICM20948_I2C_SLV4_CTRL_BANK,
                                   ICM20948_I2C_SLV4_CTRL_ADDR,
                                   0x80);
        }
    ...
    bool ICM20948_I2C::_read_mag_byte(const uint8_t mag_reg,
                                      uint8_t& byte)
    {
        bool success = true;
        ...
        if (success) {
            success &= _read_byte(ICM20948_I2C_SLV4_DI_BANK,
                                  ICM20948_I2C_SLV4_DI_ADDR,
                                  byte);
        }
