    ...
    * SOLID principles:
    *   S -   This class is responsible only for wrapping the basic GPIO-backed LED
    *         control mechanism and exposing a small interface for derived LED classes.
    *   O -   Derived classes can extend behaviour such as flashing or timed service
    *         updates without changing the base class interface.
    *   L -   Client code can use this base class interface and substitute any derived
    *         LED controller that implements the same contract.
    *   I/D - Other parts of the system can depend on this small LED abstraction rather
