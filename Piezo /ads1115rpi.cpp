...
void ADS1115rpi::start(ADS1115settings settings)
{
    ...
    if (fd_i2c < 0)
    {
        const char* i2copen = "Could not open I2C.";
#ifdef DEBUG
        fprintf(stderr, "%s\n", i2copen);
#endif
    ...
    {
        const char* i2cslave = "Could not access I2C address.";
#ifdef DEBUG
        fprintf(stderr, "%s\n", i2cslave);
#endif
...
{
    float v = static_cast<float>(i2c_readConversion()) / static_cast<float>(0x7fff) * fullScaleVoltage();
...
int ADS1115rpi::i2c_readConversion()
{
    ...
    return (static_cast<int>(tmp[0]) << 8) | static_cast<int>(tmp[1]);
}
